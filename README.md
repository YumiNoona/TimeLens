...
Version 7.8 reconciles throttled long-running web media from real playback-position progress, makes browser-coverage diagnostics reflect the period the extension actually observed, preserves the active dashboard page across refresh and browser navigation, and gives the contribution calendar a compact GitHub-style footprint.
...
The desktop, dashboard, installer, Chrome companion, and Firefox companion are released as `v7.8.0`. Vercel resolves the greatest complete stable GitHub release for every download, while installed apps discover the same separately checksummed desktop executable through the update feed. Firefox installations from Mozilla Add-ons update in place under the stable `timelens@timelens.app` ID.
...
TimeLens 7.8.0 improves long YouTube/media-session accuracy, browser diagnostics, refresh navigation, and heatmap proportions. See [7.8.0 release notes](docs/releases/7.8.0.md).
