...
Version 7.9 completes the activity-accuracy and dashboard clarity overhaul: foreground and concurrent-media time are explicitly separated, the focus calculation is documented and visualized, app/category/timeline colors adapt to every theme, the category donut is easier to read, and the public guide now explains pairing, browser attribution, picture-in-picture, coverage warnings, and troubleshooting in detail.
...
The desktop, dashboard, installer, Chrome companion, and Firefox companion are released as `v7.9.0`. Vercel resolves the greatest complete stable GitHub release for every download, while installed apps discover the same separately checksummed desktop executable through the update feed. Firefox installations from Mozilla Add-ons update in place under the stable `timelens@timelens.app` ID.
...
TimeLens 7.9.0 documents and surfaces the complete time-accounting model and refines dashboard readability across themes. See [7.9.0 release notes](docs/releases/7.9.0.md).
