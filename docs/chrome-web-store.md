# Chrome Web Store submission - TimeLens 7.9.0
...
Developer mode and choose Load unpacked. Run TimeLens 7.9.0 on the same PC.
...
- storage: keep the local pairing token and a bounded durable queue for input batches awaiting the local desktop API.
...
Privacy disclosure: browsing history (URLs, titles and domains) and aggregate user-activity counts (clicks and keystrokes) go only to
http://127.0.0.1:47821 on the user's computer and persist in the local desktop
SQLite database under configured retention. No activity is sold or sent to analytics
or advertising servers. Disclose browsing-history and user-activity handling in the store privacy form. No typed characters, key values, passwords or form contents are read. Input capture follows the desktop input setting. Undelivered batches remain in a bounded extension-local queue and drain after reconnection.
...
add-on identity. Both manifests declare 7.9.0. The Firefox package includes the same
...
Generate a pairing code in desktop Settings > Privacy center and enter it in the extension popup. Pair each browser profile separately. Store version numbers must continue increasing after 7.9.0; an already-published higher version cannot be replaced by this package.
