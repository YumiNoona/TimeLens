<script lang="ts">
  ...
  const ACTIONS: { id: Action; label: string; icon: string; description: string }[] = [
    { id: 'notify', label: 'Notify', icon: 'ti-bell', description: 'Show a repeating website reminder without closing the page.' },
  ...
  const APP_ACTIONS = ACTIONS.filter((item) => item.id !== 'notify');
  const WEB_ACTIONS = ACTIONS.filter((item) => item.id === 'notify' || item.id === 'strict');
  const DURATIONS = [
  ...
  let customDurationAmount = $state(1);
  let customDurationUnit = $state(60);
  ...
  let blockNotifyPosition = $state<'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'>('bottom-left');
  let blockMediaLayout = $state<'thumbnail' | 'large' | 'banner'>('large');
  ...
  function effectiveAction(entry: BlockEntry): Action {
    if (entry.a) return isApp(entry) && entry.a === 'notify' ? 'hide' : entry.a;
    if (!isApp(entry)) return legacyAction === 'notify' ? 'notify' : 'strict';
    return legacyAction === 'notify' ? 'hide' : legacyAction;
  }
  function actionsFor(kind: TargetKind, identifier = '') {
    if (kind === 'website') return WEB_ACTIONS;
    return isWindowsShell(identifier) ? APP_ACTIONS.filter(action => action.id === 'hide') : APP_ACTIONS;
  }
  ...
  async function load() {
    try {
      ...
      const savedPosition = settings.blockNotifyPosition === 'left' ? 'bottom-left' : settings.blockNotifyPosition === 'right' ? 'bottom-right' : settings.blockNotifyPosition;
      blockNotifyPosition = ['top-left', 'top-right', 'bottom-left', 'bottom-right'].includes(savedPosition) ? savedPosition : 'bottom-left';
      blockMediaLayout = ['thumbnail', 'large', 'banner'].includes(settings.blockMediaLayout) ? settings.blockMediaLayout : 'large';
  ...
  async function addTarget(raw = newItem) {
    ...
    const targetAction = targetKind === 'app' && (newAction === 'notify' || (isWindowsShell(value) && (newAction === 'kill' || newAction === 'strict'))) ? 'hide' : newAction;
    const durationMinutes = addingDuration === -1
      ? Math.max(1, Math.min(525600, Math.round(customDurationAmount * customDurationUnit)))
      : addingDuration;
    const entry: BlockEntry = durationMinutes > 0
      ? { i: value, m: 't', e: new Date(Date.now() + durationMinutes * 60_000).toISOString(), a: targetAction }
  ...
  async function saveNotification() {
    ...
    try {
      ...
      const response = await fetch('/api/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ blockTitle, blockMessage, blockNotifyIntervalSeconds, blockNotifyPosition, blockMediaLayout }) });
      const result = await response.json().catch(() => ({}));
...
<div class="block-page">
  ...
  <section class="focus-bar">
    ...
    <div class="focus-actions">
      {#if blockProtectionEnabled}<span class="locked"><i class="ti ti-lock"></i>Password protected</span>{/if}
  ...
  <div class="workspace">
    <section class="panel message-panel">
      ...
      <div class="preview-shell" class:browser={targetKind === 'website'}>
        <div class="preview" class:has-image={!!blockImageVersion} class:media-large={blockMediaLayout === 'large'} class:media-banner={blockMediaLayout === 'banner'}>
      ...
      <div class="notify-settings">
        ...
        <label><span>Toast corner</span><select bind:value={blockNotifyPosition} onchange={() => notificationDirty = true}><option value="top-left">Top left</option><option value="top-right">Top right</option><option value="bottom-left">Bottom left</option><option value="bottom-right">Bottom right</option></select></label>
        <label><span>Media size</span><select bind:value={blockMediaLayout} onchange={() => notificationDirty = true}><option value="thumbnail">Thumbnail</option><option value="large">Large</option><option value="banner">Full banner</option></select></label>
      </div>
    ...
    <section class="panel add-panel">
      ...
      <div class="target-picker">
        <label class="target-input"><span>{targetKind === 'app' ? 'App name' : 'Domain'}</span><div><i class="ti ti-search"></i><input bind:value={newItem} placeholder={targetKind === 'app' ? 'Search running apps' : 'youtube.com'} oninput={(event) => keepShellModeSafe(event.currentTarget.value)} onfocus={() => { showSuggestions = true; if (targetKind === 'app') void loadRunning(); }} onkeydown={(event) => { if (event.key === 'Enter') addTarget(); if (event.key === 'Escape') showSuggestions = false; }} /><button onclick={() => addTarget()} disabled={!newItem.trim() || saving}><i class="ti ti-plus"></i>Add</button></div></label>
        {#if targetKind === 'app' && showSuggestions}
          <div class="suggestions">
            ...
            {#if suggestions.length}
              {#each suggestions as process}<button type="button" onclick={() => addTarget(process)}><i class="ti {appIcon(process) || 'ti-apps'}"></i><span>{process}</span>{#if isWindowsShell(process)}<small>Hide only</small>{/if}</button>{/each}
      ...
      <div class="duration-row">
        ...
        {#if addingDuration === -1}<label class="duration custom-duration"><span>Custom duration</span><div><input type="number" min="1" max="525600" bind:value={customDurationAmount} aria-label="Custom block duration" /><select bind:value={customDurationUnit} aria-label="Custom duration unit"><option value={1}>minutes</option><option value={60}>hours</option><option value={1440}>days</option></select></div></label>{/if}
      </div>
      <div class="mode-note"><i class="ti {targetKind === 'website' ? (newAction === 'notify' ? 'ti-bell' : 'ti-shield-lock') : 'ti-device-desktop'}"></i><span>{targetKind === 'website' ? (newAction === 'notify' ? 'Shows a repeating toast in your selected corner and leaves the site usable.' : 'Shows your custom full-page screen until the target is removed.') : 'Desktop apps support Hide, Kill, and Strict.'}</span></div>
    </section>
  </div>
...
<style>
  ...
  .focus-bar,.panel{background:var(--clr-bg-sec);border:1px solid var(--clr-border);border-radius:var(--shape-lg)}.focus-bar{min-height:66px;display:flex;align-items:center;justify-content:space-between;padding:12px 16px}.focus-state,.focus-actions,.switch-label,.panel header,.image-actions,.target-row{display:flex;align-items:center}.focus-state{gap:11px}.focus-state>span{width:38px;height:38px;display:grid;place-items:center;border-radius:11px;color:var(--clr-text-sec);background:var(--clr-bg-ter)}.focus-state>span.active{color:var(--md-primary);background:var(--md-primary-cont)}.focus-state div{display:grid;gap:2px}.focus-state strong{font-size:14px;color:var(--clr-text-pri)}.focus-state small{font-size:11px;color:var(--clr-text-sec)}.focus-actions{gap:10px}.locked,.extension{display:inline-flex;align-items:center;gap:5px;padding:6px 10px;border:1px solid color-mix(in srgb,var(--md-primary) 35%,var(--clr-border));border-radius:999px;background:var(--md-primary-cont);color:var(--md-primary);font-size:10px;font-weight:600}.switch-label{gap:8px;color:var(--clr-text-pri);font-size:11px;font-weight:600}.toggle{appearance:none;width:40px;height:22px;margin:0;position:relative;border:0;border-radius:99px;background:var(--clr-border-strong);cursor:pointer}.toggle:after{content:'';position:absolute;width:18px;height:18px;top:2px;left:2px;border-radius:50%;background:white;transition:transform .18s}.toggle:checked{background:var(--md-primary)}.toggle:checked:after{transform:translateX(18px)}
  ...
  @media(max-width:500px){.mode-row{grid-template-columns:repeat(2,1fr)}.duration-row{grid-template-columns:1fr}.focus-state small{display:none}.preview-shell{padding:12px 12px 6px}.message-fields,.notify-settings,.image-actions{padding-left:12px;padding-right:12px}.image-actions{flex-wrap:wrap}.image-actions small{width:100%;margin-left:0}}
</style>
