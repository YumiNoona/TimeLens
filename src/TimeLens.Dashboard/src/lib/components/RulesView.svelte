<script lang="ts">
  ...
  const categories = ['Work', 'Development', 'Browsing', 'Communication', 'Entertainment', 'Design', 'Social', 'Media', 'System', 'Other'];
  const API = '/api/rules';
  ...
  async function addRule(pattern: string = '') {
    ...
    try {
      await fetch(API, {
        ...
        body: JSON.stringify({ pattern: p, category: newCat, patternType }),
      });
    ...
    <div class="builtin-section">
      ...
      {#each Object.entries(builtinExe) as [exe, cat]}
        <div class="rule-row builtin">
          ...
          <button class="override-btn" onclick={() => { newPattern = exe; newCat = cat.charAt(0).toUpperCase() + cat.slice(1) }} title="Override">
    ...
    <div class="builtin-section">
      ...
      {#each Object.entries(builtinDomains) as [domain, cat]}
        <div class="rule-row builtin">
          ...
          <span class="rule-arrow">→</span>
          <span class="rule-cat">{cat}</span>
          <button class="override-btn" onclick={() => { newPattern = domain; newCat = cat.charAt(0).toUpperCase() + cat.slice(1) }} title="Override">
            <i class="ti ti-pencil"></i>
          </button>
        </div>
      {/each}
