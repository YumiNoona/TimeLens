...
try
{
    ...
    try
    {
        ...
        settingsService.Save("release_notice_version", "8.1.0");
        using (var notice = System.Text.Json.JsonDocument.Parse(await dashboard.GetStringAsync("/api/release-notice")))
            Check(notice.RootElement.GetProperty("pending").GetBoolean() && notice.RootElement.GetProperty("version").GetString() == "8.1.0",
